...
class Attention(Module):
    ...
    def forward(self, x):
        ...
        q, k, v = rearrange(self.to_qkv(
            x), 'b n (qkv h d) -> qkv b h n d', qkv=3, h=self.heads)
...
class OriginalMelBandModel(torch.nn.Module, FromOriginalModelMixin):
    ...
    def __init__(
            ...
            *,
            ...
            # if True, pad output tensor to match length of input tensor
        ...
        freqs = torch.stft(torch.randn(1, 4096), **
                           self.stft_kwargs, return_complex=True).shape[1]
        ...
        repeated_freq_indices = repeat(
            torch.arange(freqs), 'f -> b f', b=num_bands)
        ...
        self.register_buffer(
            'freqs_per_band', freqs_per_band, persistent=False)

        num_freqs_per_band = reduce(freqs_per_band, 'b f -> b', 'sum')
        num_bands_per_freq = reduce(freqs_per_band, 'b f -> f', 'sum')

        self.register_buffer('num_freqs_per_band',
                             num_freqs_per_band, persistent=False)
        self.register_buffer('num_bands_per_freq',
                             num_bands_per_freq, persistent=False)
    ...
    def forward(
        ...
        raw_audio, batch_audio_channel_packed_shape = pack_one(
            raw_audio, '* t')

        stft_window = self.stft_window_fn(device=device)

        stft_repr = torch.stft(raw_audio, **self.stft_kwargs,
        ...
        masks_summed = torch.zeros_like(
            stft_repr_expanded_stems).scatter_add_(2, scatter_indices, masks)
        ...
        for window_size in self.multi_stft_resolutions_window_sizes:
            ...
            multi_stft_resolution_loss = multi_stft_resolution_loss + \
                F.l1_loss(recon_Y, target_Y)

        weighted_multi_resolution_loss = multi_stft_resolution_loss * \
            self.multi_stft_resolution_loss_weight
