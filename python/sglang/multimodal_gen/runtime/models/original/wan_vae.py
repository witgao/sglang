...
class OriginalWanVAEModel(torch.nn.Module, FromOriginalModelMixin):

    def __init__(
        self, z_dim=16, dtype=torch.float32, pruning_rate=0.0
    ):
        super().__init__()
        self.dtype = dtype
    ...
    def from_single_file(cls, pretrained_model_link_or_path_or_dict=None, **kwargs):
        ...
        vae_sd = ModelUtils.load_torch_file(
        ...
        if dim == 96:
            logging.info("Detected lightVAE model with 75% pruning")
            pruning_rate = 0.75
        else:
            pruning_rate = 0.0

        if vae_sd["model.conv2.weight"].shape[0] == 16:
            vae = OriginalWanVAEModel(dtype=dtype, pruning_rate=pruning_rate)
        ...
        if compile_args is not None:
            vae.model.decoder = torch.compile(
                vae.model.decoder, fullgraph=compile_args["fullgraph"], dynamic=compile_args["dynamic"], backend=compile_args["backend"], mode=compile_args["mode"])

        return vae

EntryClass = [OriginalWanVAEModel]
