...
from sglang.comfy.utils import load_torch_file, ProgressBar
import sglang.comfy.model_base
...
def update_folder_names_and_paths(key, targets=[]):
    ...
    base = sglang.comfy.folder_paths.folder_names_and_paths.get(key, ([], {}))
    base = base[0] if isinstance(base[0], (list, set, tuple)) else []
    # find base key & add w/ fallback, sanity check + warning
    target = next((x for x in targets if x in sglang.comfy.folder_paths.folder_names_and_paths), targets[0])
    orig, _ = sglang.comfy.folder_paths.folder_names_and_paths.get(target, ([], {}))
    sglang.comfy.folder_paths.folder_names_and_paths[key] = (orig or base, {".gguf"})
